...
**display_participant_set_infos** | [**\Adobe\ESign\Agreements\Model\DisplayParticipantSetInfo[]**](DisplayParticipantSetInfo.md) | The most relevant current user set for the agreement. It is typically the next signer if the agreement is from the current user, or the sender if received from another user | [optional] 
